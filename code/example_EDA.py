...
####################################################
data = pd.read_csv(
    '/Users/tomas/CompBME/Module4_Vikesland+Daniel/Module-4-Cancer/data/TRAINING_SET_GSE62944_subsample_log2TPM.csv', index_col=0, header=0)  # can also use larger dataset with more genes
metadata_df = pd.read_csv(
    '/Users/tomas/CompBME/Module4_Vikesland+Daniel/Module-4-Cancer/data/TRAINING_SET_GSE62944_metadata.csv', index_col=0, header=0)
print(data.head())
...
####################################################
cancer_type = 'LUAD'  # Breast Invasive Carcinoma
...
####################################################
desired_gene_list = ['HMMR', 'B4GALT1', 'SLC16A3', 'ANGPTL4', 'EXT1', 'GPC1', 'RBCK1', 'SOD1', 'AGRN']
...
BRCA_gene_data = LUAD_data.loc[gene_list]
print(LUAD_data.head())

# %%
# Basic statistics on the subsetted data
####################################################
print(LUAD_data.describe())
print(LUAD_data.var(axis=1))  # Variance of each gene across samples
...
sns.boxplot(data=BRCA_merged, x="gender", y='HMMR')
plt.title("HMMR Expression by Gender in BRCA Samples")
plt.show()

# Boxplot of MYC and EGFR expression in BRCA samples using PANDAS directly
BRCA_merged[['RBCK1', 'HMMR']].plot.box()
plt.title("RBCK1 and HMMR Expression in BRCA Samples")
plt.show()
